...
public class ScriptMachine
{
	...
	private ScriptVariable ThisObject => _receiverOverride ?? _script;
	private ScriptVariable? RefObject => _receiverOverride ?? _script.RefObject;
	...
	public async Task<IStackEntry> Execute(string functionName, Stack<IStackEntry>? callStack = null, ScriptVariable? receiverOverride = null)
	{
		...
		while (index < _script.Bytecode.Length)
		{
			...
			switch (op.OpCode)
			{
				...
				case Opcode.OP_OR:
					...
					var orCompEntry = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
				...
				case Opcode.OP_AND:
					...
					var andCompEntry = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
				...
				case Opcode.OP_CALL:
					...
					while (parameters.Count > 0 && parameters.Peek().Type != ArrayStart)
					{
					...
					if (callEntry.Type is StackEntryType.String or Variable &&
					    opWith is { Count: > 0 } &&
					    TryGetWithFunctionEntry(opWith.Peek(), cmd?.ToString() ?? string.Empty, out var withFunctionEntry))
					{
					...
					switch (callEntry.Type)
					{
							case StackEntryType.String or Variable
								...
								stack.Push(await Execute(cmd?.ToString()?.ToLowerInvariant() ?? string.Empty, new(callParams)).ConfigureAwait(false));
							break;
						case StackEntryType.String or Variable
							...
							switch (thisFunctionEntry.Type)
							{
								...
								case ScriptProperty:
									...
									var thisFunctionInstance = thisFunctionProperty != null
										...
										: null;
									stack.Push((thisFunctionProperty?.Call(thisFunctionInstance!, callParams.ToArray()) ?? 0).ToStackEntry());
									break;
								default:
						...
						*/
						...
						case ScriptProperty:
							...
							if (scriptProperty != null)
								inst = ResolveScriptPropertyInstance(scriptProperty, inst ?? callEntry.GetParent());
				...
				case Opcode.OP_TYPE_VAR:
					stack.Push((op.VariableName?.ToString().ToLowerInvariant() ?? "").ToStackEntry(true));
					break;
				...
				case Opcode.OP_MEMBER_ACCESS:
					...
					try
					{
						...
						if (stack.Peek()?.Type == StackEntryType.Script)
						{
							...
							{
								var prop = scriptObject?.Properties.FirstOrDefault(x => x.PropertyName.Equals(
									memberAccessParam ?? string.Empty,
									StringComparison.CurrentCultureIgnoreCase
								));
								if (prop != null)
								{
				...
				case Opcode.OP_ARRAY_NEW:
					var arraySize = ClampScriptArraySize(ToScriptInt(stack.Pop().GetValue<double>()));
					stack.Push(CreateScriptArray(arraySize).ToStackEntry());
					break;
				case Opcode.OP_SETARRAY:
					var setArraySize = ClampScriptArraySize(ToScriptInt(GetEntry(stack.Pop()).GetValue<double>()));
					var setArray     = GetMutableScriptArray(GetEntry(stack.Pop(), returnStackEntryIfNotFound: true));
					if (setArray != null)
				...
				case Opcode.OP_MAKEVAR:
					var makeVarName = GetEntryValue<TString>(stack.Pop())?.ToString() ?? string.Empty;
				...
				case Opcode.OP_NEW_OBJECT:
					...
					try
					{
						var newObjectClassName = GetRawStackString(newObject);
						var newObjectName      = GetRawStackString(newObjectParam);
				...
				case Opcode.OP_OBJ_FROM_STR:
					var objFromStrName = GetEntryValue<TString>(stack.Pop())?.ToString() ?? string.Empty;
						...
						try
						{
							if (callStack is { Count: > 0 })
							{
								...
								_tempVariables.AddOrUpdate(
										(funcParam.GetValue() ?? "").ToString()?.ToLowerInvariant() ?? string.Empty,
				...
				case Opcode.OP_UNKNOWN_54:
					if (stack.Count >= 3)
					{
						...
						var memberAssignTarget = GetMemberValue(stack.Pop(), memberAssignName);
				...
				case Opcode.OP_UNKNOWN_227:
					var optimizedCompareLeft  = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
					var optimizedCompareRight = op.Value.ToStackEntry();
				...
				case Opcode.OP_UNKNOWN_232:
					if (callStackRegisters.TryGetValue(ToScriptInt(op.Value), out var registerEntry))
						IncrementEntry(registerEntry, op.OpCode == Opcode.OP_UNKNOWN_231 ? 1.0d : -1.0d);
					break;
				...
				case Opcode.OP_UNKNOWN_240:
					...
					stack.Push((GetScriptArrayCell(optimizedArrayTarget, ToScriptInt(op.Value)) ?? 0.0d).ToStackEntry());
					break;
				...
				case Opcode.OP_UNARYSUB:
					...
					break;
				case Opcode.OP_EQ:
					var eqRight = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
					var eqLeft  = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
					stack.Push(ScriptEntriesEqual(eqLeft, eqRight).ToStackEntry());
					break;
				case Opcode.OP_NEQ:
					var neqRight = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
					var neqLeft  = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
					stack.Push((!ScriptEntriesEqual(neqLeft, neqRight)).ToStackEntry());
					break;
				case Opcode.OP_LT:
					var ltRight = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
					var ltLeft  = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
					stack.Push((CompareScriptValues(ltLeft, ltRight) < 0).ToStackEntry());
					break;
				case Opcode.OP_GT:
					var gtRight = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
					var gtLeft  = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
					stack.Push((CompareScriptValues(gtLeft, gtRight) > 0).ToStackEntry());
					break;
				case Opcode.OP_LTE:
					var lteRight = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
					var lteLeft  = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
					stack.Push((CompareScriptValues(lteLeft, lteRight) <= 0).ToStackEntry());
					break;
				case Opcode.OP_GTE:
					var gteRight = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
					var gteLeft  = GetEntry(stack.Pop(), returnStackEntryIfNotFound: true);
				...
				case Opcode.OP_IN_RANGE:
					...
					stack.Push((rangeValues is { Count: > 0 }
						? rangeValues.All(value => ValueInRange(ToScriptDouble(value), rangeStart, rangeEnd, rangeMode))
				...
				case Opcode.OP_IN_OBJ:
					...
					stack.Push((inObjValues != null && (inNeedleValues is { Count: > 0 }
						? inNeedleValues.All(value => IndexOfScriptValue(inObjValues, value) >= 0)
				...
				case Opcode.OP_ARRAY:
					...
					stack.Push(GetScriptArrayCell(array, arrayIndex).ToStackEntry());
					break;
				...
				case Opcode.OP_ARRAY_MULTIDIM:
					...
					stack.Push(GetScriptArrayCell2(multiArray, multiArrayX, multiArrayY).ToStackEntry());
					break;
				...
				case Opcode.OP_ARRAY_NEW_MULTIDIM:
					...
					case Opcode.OP_WITH:
						...
						if (withValue is null || withTarget.Type == StackEntryType.Number && ToScriptDouble(withValue) == 0.0d)
					{
						index = (int)op.Value;
						_indexPos = index;
							break;
				...
				case Opcode.OP_FOREACH:
					...
					break;
					case Opcode.OP_THIS:
						stack.Push(ThisObject.ToStackEntry());
						break;
				...
				case Opcode.OP_PARAMS:
					stack.Push(_tempVariables.GetVariable("params"));
					break;
	...
	{
		var parameters = callStack?
			.ToArray()
			.Select(entry => UnwrapScriptValue(entry.GetValue()))
			.Cast<object?>()
			.ToList() ?? [];

		_tempVariables.AddOrUpdate("params", parameters.ToStackEntry());
	}
	...
	private IStackEntry ConvertToObjectEntry(IStackEntry entry, Stack<IStackEntry>? opWith)
	{
		...
		return GetEntry(variableName.ToString().ToStackEntry(true), Variable);
	}

	private IStackEntry GetMemberValue(IStackEntry parentStackEntry, string memberName)
	{
		...
		try
		{
			...
			if (parentValue is Script script)
			{
				...
				var property = script.Properties.FirstOrDefault(prop =>
					prop.PropertyName.Equals(normalizedMemberName, StringComparison.CurrentCultureIgnoreCase));
				if (property != null)
				...
				return script.GetVariable(normalizedMemberName);
			}
			...
			if (parentValue is VariableCollection variableCollection)
			{
				...
				{
					var prop = scriptVariable.Properties.FirstOrDefault(x =>
						x.PropertyName.Equals(normalizedMemberName, StringComparison.CurrentCultureIgnoreCase));
					if (prop != null)
	...
	private bool TryGetWithMemberEntry(IStackEntry parentStackEntry, string memberName, out IStackEntry memberEntry)
	{
		...
		if (parentValue is ScriptVariable scriptVariable)
		{
			var property = scriptVariable.Properties.FirstOrDefault(prop =>
				prop.PropertyName.Equals(normalizedMemberName, StringComparison.CurrentCultureIgnoreCase));
			if (property != null)
			{
	...
	private void IncrementEntry(IStackEntry entry, double delta)
	{
		var target = entry.Type == Variable
			? GetOrCreateScriptVariable(entry.GetValue()?.ToString() ?? string.Empty)
			: GetEntry(entry, returnStackEntryIfNotFound: true);
		...
		switch (type)
		{
			case Variable when stackEntry.GetParent() is VariableCollection parentCollection:
				retVal = parentCollection.GetVariable(
					NormalizeScriptVariableName(stackEntry.GetValue()?.ToString() ?? string.Empty)
				);
				_useTemp = false;
				foundVariable = true;
				break;
			...
			case Variable when ScriptManager.GlobalProperties[nameof(Script)].Any(x => x.PropertyName.Equals(stackEntry.GetValue()?.ToString(), StringComparison.CurrentCultureIgnoreCase)):
				...
				break;
			case Variable
					when _tempVariables.ContainsVariable(stackEntry.GetValue()?.ToString()?.ToLowerInvariant() ?? string.Empty):
					_useTemp = false;
