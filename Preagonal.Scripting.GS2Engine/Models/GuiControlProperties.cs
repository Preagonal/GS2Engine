...
public class GuiControlProperties : ScriptProperties<GuiControl>
{
	public GuiControlProperties() : base(typeof(ScriptVariable))
	{
		...
		propertyDefinitions.Add<object?>("profile", "The visual profile used by the control.", control => control.Profile, (control, profile) => control.Profile = GetValue(profile));
		...
		functionDefinitions.Add<int>("addcontrol", "Adds a child control.",
		                                 (control, o2) =>
		                                 {
		...
		functionDefinitions.Add<int>("bringtofront", "Moves the control in front of its siblings.", (control, _) => { control.BringToFront(); return 0; });
		functionDefinitions.Add<int>("clearcontrols", "Removes all child controls.", (control, _) => { control.ClearControls(); return 0; });
		...
		functionDefinitions.Add<int>("destroy", "Destroys the control.", (control, _) => { control.Destroy(); return 0; });
		...
		functionDefinitions.Add<int>("hide", "Hides the control.", (control, _) => { control.Hide(); return 0; });
		...
		functionDefinitions.Add<int>("makefirstresponder", "Sets or clears keyboard focus for the control.", (control, args) => { control.MakeFirstResponder(GetBool(args, 0)); return 0; }, [new("active", typeof(bool))]);
		functionDefinitions.Add<int>("mouselock", "Locks the specified mouse button to the control.", (control, args) => { control.MouseLock(GetInt(args, 0)); return 0; }, [new("button", typeof(int))]);
		functionDefinitions.Add<int>("mouseunlock", "Unlocks the specified mouse button from the control.", (control, args) => { control.MouseUnlock(GetInt(args, 0)); return 0; }, [new("button", typeof(int))]);
		functionDefinitions.Add<int>("mouseunlockall", "Releases every mouse-button lock held by the control.", (control, _) => { control.MouseUnlockAll(); return 0; });
		functionDefinitions.Add<int>("pushtoback", "Moves the control behind its siblings.", (control, _) => { control.PushToBack(); return 0; });
		functionDefinitions.Add<int>("resize", "Changes the control position and size.",
		                             (control, args) =>
		                             {
		...
		functionDefinitions.Add<int>("repaint", "Requests that the control be redrawn.", (control, _) => { control.Repaint(); return 0; });
		functionDefinitions.Add<int>("settext", "Sets the control text.", (control, args) => { control.Text = GetString(args, 0); return 0; }, [new("text", typeof(string))]);
		functionDefinitions.Add<int>("show", "Shows the control.", (control, _) => { control.Show(); return 0; });
		functionDefinitions.Add<int>("showtop", "Shows the control in front of its siblings.", (control, _) => { control.ShowTop(); return 0; });
		functionDefinitions.Add<int>("showAlwaysTop", "Shows the control and keeps it above sibling controls.", (control, _) => { control.ShowAlwaysTop(); return 0; });
		functionDefinitions.Add<int>("sortcontrols", "Sorts the child controls by their display order.", (control, _) => { control.SortControls(); return 0; });
		functionDefinitions.Add<int>("startdrag", "Begins dragging the control.", (control, _) => { control.StartDrag(); return 0; });
		functionDefinitions.Add<int>("stopanimations", "Stops the control's active animations.", (control, _) => { control.StopAnimations(); return 0; });
		functionDefinitions.Add<int>("stopinoutanimations", "Stops the control's active in/out animations.", (control, _) => { control.StopInOutAnimations(); return 0; });
